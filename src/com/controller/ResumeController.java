...
public class ResumeController {
    ...
    public String resumeadd(ModelAndView mv,@ModelAttribute Resume resume ,Integer id,
		 ...
		 */
		 ...
		 //将投递简历的信息存入数据库
		 ahualyService.insert_Resume(resume);
    ...
    public String recruitment(Integer pageIndex,Model model,String content,Resume resume) {
		...
		List<Resume> resume_list = ahualyService.get_ResumeList(resume,pageModel);
		Integer count = 0;
		if (content!=null&&!content.equals("")){
			count = ahualyService.countResume(content);
			resume_list = ahualyService.get_ResumeLikeList(content);
		}
       ...
       //简历审核跳转页面  /resume/edit
  		@RequestMapping(value="/resume/edit",method=RequestMethod.GET)
  		 public String edit(Model model,Integer id){
  			if(id!=null){
  				Resume resume = ahualyService.get_ResumeInfo(id);
  		...
  		public String toedit(Integer id,String email,String name,Integer status_id) {
  			...
  			ahualyService.update_ResumeInfo(resume);
		...
		@RequestMapping(value="/resume/downLoad")
		 public ResponseEntity<byte[]>  downLoad(
			...
			// 根据id查询文档
			Resume target = ahualyService.get_ResumeInfo(id);
